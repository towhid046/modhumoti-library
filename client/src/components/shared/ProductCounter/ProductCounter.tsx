...
const ProductCounter: React.FC<ProductCounterProps> = ({ book }) => {
    ...
    return (
        <div className="flex items-center border border-gray-300 rounded-md justify-between">
            <button
                className={`${path.includes('cart') ? 'h-8 w-7' : 'h-10 w-9'} flex items-center justify-center rounded-l-md disabled:cursor-not-allowed border-r border-gray-300 hover:bg-gray-100 transition-colors duration-200`}
